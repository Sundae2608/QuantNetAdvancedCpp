...
template <class TYPE>
class Destroyer { // Class which is responsible for the destruction of another
    // type TYPE
...
private:
    ...
    Destroyer<TYPE>&
        operator=(const Destroyer<TYPE>& source); // Assignment operator
...
public:
    ...
    // Somehow the default constructor must be public else we got a compiler error
    // in class ctype
    Destroyer(); // Default constructor
};

template <class TYPE> class Singleton { // Templated Singleton class
